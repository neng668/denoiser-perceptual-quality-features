...
class Solver(object):
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(logprog):
            ...
            logprog.update(loss=format(total_loss / (i + 1), ".5f"), L1_loss=format(L1_loss / (i + 1), ".5f"),
                           sc_loss=format(sc_loss / (i + 1), ".5f"), mag_loss=format(mag_loss / (i + 1), ".5f"), mag_s_loss=format(mag_s_loss / (i + 1), ".5f"),
                           mfcc_loss=format(mfcc_loss / (i + 1), ".5f"), mfcc_std=format(mfcc_std / (i + 1), ".5f"),
                           std_loss=format(std_loss / (i + 1), ".5f"), skw_loss=format(skw_loss / (i + 1), ".5f"), krt_loss=format(krt_loss / (i + 1), ".5f"))#, lpc_skw=format(lpc_skw / (i+1), ".5f"))
            #logprog.update(loss=format(total_loss / (i + 1), ".5f"), L1_loss=format(L1_loss / (i + 1), ".5f"),
            #               sc_loss=format(sc_loss / (i + 1), ".5f"), mag_loss=format(mag_loss / (i + 1), ".5f"), mag_s_loss=format(mag_s_loss / (i + 1), ".5f"),
            #              std_loss=format(std_loss / (i + 1), ".5f"), skw_loss=format(skw_loss / (i + 1), ".5f"), krt_loss=format(krt_loss / (i + 1), ".5f"))
